  ...
  <div class="page-wrapper">
	...
	<div class="widget {{d[i].style}}">
				...
				{{# for(var y = 0, ilen = d[i].data.length; y < ilen; y++){}}
                   ...
                   {{#} }}
                     {{#  if(d[i].title=='商品分类'){}}
                     <li class="companychange"><a href="#" id="showtype{{d[i].data[y].id}}" class="widgetb  {{# if(y==0){}}allpricechange{{#} }}" onClick="showlist({{d[i].data[y].id}},'showtype')">{{d[i].data[y].name}}</a></li>
                   {{#} }}
