...
$lists = $user_list['user_list'];


 foreach ($lists as $k=>$v){
     $sqtime = $lists[$k]['time'];  //申请时间
    ...
    $user_list['user_list'][$k]['datetime'] = $timesyhanzi;
    ...
    $smarty->display('tixian_list.htm');
...
$lists = $user_list['user_list'];
 ...
 foreach ($lists as $k=>$v){
    ...
    make_json_result($smarty->fetch('tixian_list.htm'), '', array('filter' => $user_list['filter'], 'page_count' => $user_list['page_count']));
...

{
...

    {
      ...
      $filter['status'] = $_REQUEST['status'];
        ...
        if ($filter['status']!=5 &&isset($filter['status']))
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('tixian')."as t " .
            "INNER JOIN".$GLOBALS['ecs']->table('users')."as u
            on t.line_shopid = u.user_id" .$ex_where." order by t.`status`,t.time desc"
            );
     ...
     $sql  = 'SELECT t.id,u.user_name,t.money,t.time,t.`status` FROM ' .

            $GLOBALS['ecs']->table('tixian'). ' AS t INNER JOIN ' .

            $GLOBALS['ecs']->table('users'). ' AS u ON t.line_shopid = u.user_id'.

            $ex_where . " ORDER by t.`status`,t.time " .$filter['sort_order']. " LIMIT ".$filter['start'].", ".$filter['page_size'];
