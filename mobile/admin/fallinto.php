...

{
    ...
    make_json_result($smarty->fetch('delivery_listfc.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
...

{
    ...
    $smarty->display('delivery_listfc.htm');
...

{
    ...
    make_json_result($smarty->fetch('deliveryfc_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
...
//     //获取分页总数
//     $sql = "select count(*),f.id,eu1.user_name as xxdname,g.goods_name,eu2.user_name as username,f.money,f.type,
//     eu3.user_name as tcusername
//     from `ecs_fencheng` as f
//     inner join `ecs_users` as eu1 on f.line_shopid = eu1.user_id
//     inner join `ecs_goods` as g on f.goodsid = g.goods_id
//     inner join `ecs_users`as eu2 on f.userid = eu2.user_id
//     inner join `ecs_users` as eu3 on f.get_shopid = eu3.user_id
//     where eu1.is_line = 1";
//     $filter['record_count'] = $GLOBALS['db']->getOne($sql);
...
//  $sql = 'SELECT f.id,eu1.user_name as xxdname,g.goods_name,eu2.user_name as username,f.money,f.type,
//     eu3.user_name as tcusername '.

//             'FROM ' .$GLOBALS['ecs']->table('fencheng'). 'AS f ' .

//             'LEFT JOIN ' . $GLOBALS['ecs']->table('users'). ' AS eu1 ON f.line_shopid = eu1.user_id '.

//             'LEFT JOIN ' . $GLOBALS['ecs']->table('goods'). ' AS g   ON f.goodsid = g.goods_id ' .

//             'LEFT JOIN ' . $GLOBALS['ecs']->table('users'). ' AS eu2 ON f.userid = eu2.user_id ' .

//             'LEFT JOIN ' . $GLOBALS['ecs']->table('users'). ' AS eu3 ON f.get_shopid = eu3.user_id ' .
