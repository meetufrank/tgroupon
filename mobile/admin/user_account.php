...

{
    ...
    $list['list'][$k]['datetime'] = $timesyhanzi;

   }
   // print_r($list['filter']);exit;
...

{
...

    {
        ...
        $sql = "select count(*) from `ecs_tixian`  as t
inner join `ecs_users` as u
on t.line_shopid = u.user_id order by t.`status`,t.time desc " . $where;
        ...
        $sql  = 'SELECT t.id,u.user_name,t.money,t.time,t.`status` FROM ' .
            ...
            $where . " ORDER by t.`status`,t.time " .$filter['sort_order']. " LIMIT ".$filter['start'].", ".$filter['page_size'];
