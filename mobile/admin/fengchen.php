...

{
    ...
    $fclistsql = "select f.id,eu1.user_name as xxdname,g.goods_name,eu2.user_name as username,f.money,f.type,eu3.user_name as tcusername from `ecs_fencheng` as f
...
inner join `ecs_users` as eu3

on f.get_shopid = eu3.user_id

where eu1.is_line = 1";

    $fclist = $db->getAll($fclistsql);

    // print_r($fclist);exit;

    $smarty->assign('fclist',$fclist);
...

{
...

    {
        ...
        $sql = "select count(*) from `ecs_fencheng` as f
...
// print_r($filter['start']);exit;

        /* 获取文章数据 */

        $sql = 'SELECT f.id,eu1.user_name as xxdname,g.goods_name,eu2.user_name as username,f.money,f.type,eu3.user_name as tcusername '.

               'FROM ' .$GLOBALS['ecs']->table('fencheng'). ' AS f '.

               'INNER JOIN ' .$GLOBALS['ecs']->table('users'). ' AS eu1 ON f.line_shopid = eu1.user_id '.

               'INNER JOIN ' .$GLOBALS['ecs']->table('goods'). ' AS g ON f.goodsid = g.goods_id '.

               'INNER JOIN ' .$GLOBALS['ecs']->table('users'). ' AS eu2 ON f.userid = eu2.user_id '.

               'INNER JOIN ' .$GLOBALS['ecs']->table('users'). ' AS eu3 ON f.get_shopid = eu3.user_id '.

               'WHERE eu1.is_line = 1  ' ." LIMIT " . $filter['start'] . ',' . $filter['page_size'];
