...

{
    ...
    $smarty->display('shenqing_list.htm');
...

{
    ...
    make_json_result($smarty->fetch('shenqing_list.htm'), '', array('filter' => $user_list['filter'], 'page_count' => $user_list['page_count']));
...

{
    ...
    $sql = "select * from `ecs_shenqing` where sq_id = '$_GET[id]'";



    $row = $db->GetRow($sql);

    // print_r($row['sq_userid']);exit;


    $smarty->assign('sqedit', $row);

    $smarty->display('shenqing_info.htm');
...
elseif ($_REQUEST['act'] == 'xiugai'){
   $sqid = $_POST['sqid'];
   $sq_status = $_POST['radioname'];
   // print_r($sqid);exit;

   $sqstatusedit = "update `ecs_shenqing` set sq_status=$sq_status  where sq_id = $sqid";
